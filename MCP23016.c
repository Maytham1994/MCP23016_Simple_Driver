...
* Revision Date: February 10th, 2015
*/
#include <Wire.h>
...
*/
void MCP_SetDirection(unsigned char GP0State, unsigned char GP1State){
	// Update the two CurrentState parameters
	GP0CurrentState = GP0State;
	GP1CurrentState = GP1State;
...
void Motor_Directions(bool AIN1, bool AIN2, bool BIN1, bool BIN2){
	...
	unsigned char array[] = MCP_ReadPorts(0);
	// Save the port in a temporary direction variable
	unsigned char direction = array[0];
...
*/
unsigned char[] MCP_ReadPorts(bool port){
